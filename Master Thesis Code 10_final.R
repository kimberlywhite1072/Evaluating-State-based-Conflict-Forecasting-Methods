...
### Objective: Plot prediction error for Models A2, B2, and C2
...
########## DATA PREPARATION FOR MODEL A2 / B2 / C2 ##########
...
subset_deadly <- subset(subset, future_ged_best_sb > 30)
...
for(s in s_values) {
  ...
  # Set up the formula
  formula <- future_ged_best_sb ~ month_id +
    factor(month) +
...
########## CROSS VALIDATION GRAPH FOR MODEL B2 ##########
...
# Count the number of observations for each unique value in the 'bin' column
bin_counts_B2 <- subset %>%
  group_by(bin) %>%
  summarise(bin_count = n())

# Join the 'bin_counts_B2' data frame with the original dataframe
subset <- left_join(subset, bin_counts_B2, by = "bin")

# Calculate the cumulative percentage of observations
bin_counts_B2$bin_percentage <- cumsum(bin_counts_B2$bin_count) / sum(bin_counts_B2$bin_count)
print(bin_counts_B2)

# X-axis with minimum of 50
bin_counts_B2 <- subset(bin_counts_B2, bin > 0 & bin <= 50)
graph_B2 <- ggplot(data = bin_counts_B2, aes(x = bin, y = bin_percentage)) +
  geom_line(color = "steelblue", size = 0.75) +
  labs(x = "Bin", y = "Cumulative Percentage of Observations") +
  scale_x_continuous(breaks = seq(0, 50, by = 1), labels = seq(0, 50, by = 1)) +
  scale_y_continuous(labels = scales::percent_format()) +
...
########## CROSS VALIDATION GRAPH FOR MODEL C2 ##########
...
# Count the number of observations for each unique value in the 'bin' column
bin_counts_C2 <- subset %>%
  group_by(bin) %>%
  summarise(bin_count = n())

# Join the 'bin_counts_C2' data frame with the original dataframe
subset <- left_join(subset, bin_counts_C2, by = "bin")

# Calculate the cumulative percentage of observations
bin_counts_C2$bin_percentage <- cumsum(bin_counts_C2$bin_count) / sum(bin_counts_C2$bin_count)
print(bin_counts_C2)

# X-axis with minimum of 50
bin_counts_C2 <- subset(bin_counts_C2, bin > 0 & bin <= 50)
graph_C2 <- ggplot(data = bin_counts_C2, aes(x = bin, y = bin_percentage)) +
  geom_line(color = "steelblue", size = 0.75) +
  labs(x = "Bin", y = "Cumulative Percentage of Observations") +
  scale_x_continuous(breaks = seq(0, 50, by = 1), labels = seq(0, 50, by = 1)) +
  scale_y_continuous(labels = scales::percent_format()) +
...
########## ANALYSIS OF MORE DEADLY CONFLICTS FOR MODELS A2, B2, C2 ##########
subset_analysis_A$model <- "Model A2"
subset_analysis_A <- subset(subset_analysis_A, select = -c(bin, diff))

subset_analysis_B$model <- "Model B2"
subset_analysis_B <- subset(subset_analysis_B, select = -c(bin, diff))

subset_analysis_C$model <- "Model C2"
...
# Create the plot
deadly <- ggplot(subset_analysis, aes(x = future_ged_best_sb, y = predictions, color = model)) +
  geom_point() +
  labs(x = 'Number of Fatalities', y = 'Predicted Fatalities', title = 'Dataset 2: Evaluating Non-Zero Prediction Accuracy of Deadlier Conflicts') +
  scale_color_manual(values = c("Model A2" = "blue", "Model B2" = "red", "Model C2" = "green")) +
  theme_minimal() +
  theme(
  ...
  scale_x_continuous(breaks = c(30, seq(40, max(subset_analysis$future_ged_best_sb), by = 10)),
                     labels = c(30, seq(40, max(subset_analysis$future_ged_best_sb), by = 10)))
...
########## CROSS VALIDATION GRAPH FOR MODELS A2, B2, C2 ##########

### Static Plot
combined_graph <- ggplot() +
  geom_line(data = bin_counts_A2, aes(x = bin, y = bin_percentage, color = "Model A2"), size = 1) +
  geom_line(data = bin_counts_B2, aes(x = bin, y = bin_percentage, color = "Model B2"), size = 1) +
  geom_line(data = bin_counts_C2, aes(x = bin, y = bin_percentage, color = "Model C2"), size = 1) +
  labs(x = "Prediction Error", y = "Percentage of Correct Predictions", title = "Baseline Dataset + New Covariates", color = "") +
  ...
  scale_color_manual(values = c("Model A2" = "steelblue", "Model B2" = "red", "Model C2" = "green")) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
combined_graph <- ggplot() +
  ...
  geom_line(data = bin_counts_B2, aes(x = bin, y = bin_percentage, color = "Model B2"), size = 1) +
  geom_line(data = bin_counts_C2, aes(x = bin, y = bin_percentage, color = "Model C2"), size = 1) +
  labs(x = "Prediction Error", y = "Percentage of Correct Predictions", title = "Baseline Dataset + New Covariates",
       color = "") +
  scale_x_continuous(breaks = seq(1, 50, by = 1), labels = paste0("±", abs(seq(1, 50, by = 1)))) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.0025), labels = scales::percent(seq(0, 1, by = 0.0025))) +
  scale_color_manual(values = c("Model A2" = "steelblue", "Model B2" = "red", "Model C2" = "green")) +
...
htmlwidgets::saveWidget(interactive_graph, file = "Dataset 2 Baseline Dataset + New Covariates with all Models.html")
