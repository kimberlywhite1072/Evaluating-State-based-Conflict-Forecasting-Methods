...
### Objective: Plot prediction error for Models C1, C2, and C3 (Gradient Boosting Machines)
...
########## DATA PREPARATION FOR MODEL C1 / MODEL C2 ##########
...
for(s in s_values) {
  ...
  for (i in 1:k) {
    ...
    # Fit the Gradient Boosting Machine model on the training data
...
########## CROSS VALIDATION GRAPH FOR MODELS C1, C2, C3 ##########
...
combined_graph <- ggplot() +
  ...
  labs(x = "Prediction Error", y = "Percentage of Correct Predictions", title = "Hierarchical Hurdle Model with GBM",
       color = "") +
  ...
  scale_color_manual(values = c("Model C1" = "steelblue", "Model C2" = "red", "Model C3" = "green")) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
htmlwidgets::saveWidget(interactive_graph, file = "Part C - Hierarchical Hurdle Model with GBM.html")
