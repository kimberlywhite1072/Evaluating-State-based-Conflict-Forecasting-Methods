...
### Objective: Plot prediction error for Models A1, B1, and C1
...
########## DATA PREPARATION FOR MODEL A1 / B1 / C1 ##########
...
########## CROSS VALIDATION GRAPH FOR MODEL B1 ##########
...
# Count the number of observations for each unique value in the 'bin' column
bin_counts_B1 <- subset %>%
  group_by(bin) %>%
  summarise(bin_count = n())

# Join the 'bin_counts_B1' data frame with the original dataframe
subset <- left_join(subset, bin_counts_B1, by = "bin")

# Calculate the cumulative percentage of observations
bin_counts_B1$bin_percentage <- cumsum(bin_counts_B1$bin_count) / sum(bin_counts_B1$bin_count)
print(bin_counts_B1)

# X-axis with minimum of 50
bin_counts_B1 <- subset(bin_counts_B1, bin > 0 & bin <= 50)
graph_B1 <- ggplot(data = bin_counts_B1, aes(x = bin, y = bin_percentage)) +
  geom_line(color = "steelblue", size = 0.75) +
  labs(x = "Bin", y = "Cumulative Percentage of Observations") +
  scale_x_continuous(breaks = seq(0, 50, by = 1), labels = seq(0, 50, by = 1)) +
  scale_y_continuous(labels = scales::percent_format()) +
...
########## CROSS VALIDATION GRAPH FOR MODEL C1 ##########
...
# Count the number of observations for each unique value in the 'bin' column
bin_counts_C1 <- subset %>%
  group_by(bin) %>%
  summarise(bin_count = n())

# Join the 'bin_counts_C1' data frame with the original dataframe
subset <- left_join(subset, bin_counts_C1, by = "bin")

# Calculate the cumulative percentage of observations
bin_counts_C1$bin_percentage <- cumsum(bin_counts_C1$bin_count) / sum(bin_counts_C1$bin_count)
print(bin_counts_C1)

# X-axis with minimum of 50
bin_counts_C1 <- subset(bin_counts_C1, bin > 0 & bin <= 50)
graph_C1 <- ggplot(data = bin_counts_C1, aes(x = bin, y = bin_percentage)) +
  geom_line(color = "steelblue", size = 0.75) +
  labs(x = "Bin", y = "Cumulative Percentage of Observations") +
  scale_x_continuous(breaks = seq(0, 50, by = 1), labels = seq(0, 50, by = 1)) +
  scale_y_continuous(labels = scales::percent_format()) +
...
########## ANALYSIS OF MORE DEADLY CONFLICTS FOR MODELS A1, B1, C1 ##########
subset_analysis_A$model <- "Model A1"
subset_analysis_A <- subset(subset_analysis_A, select = -c(bin, diff))

subset_analysis_B$model <- "Model B1"
subset_analysis_B <- subset(subset_analysis_B, select = -c(bin, diff))

subset_analysis_C$model <- "Model C1"
...
deadly <- ggplot(subset_analysis, aes(x = future_ged_best_sb, y = predictions, color = model)) +
  geom_point() +
  labs(x = 'Number of Fatalities', y = 'Predicted Fatalities', title = 'Dataset: Evaluating Non-Zero Prediction Accuracy of Deadlier Conflicts') +
  scale_color_manual(values = c("Model A1" = "blue", "Model B1" = "red", "Model C1" = "green")) +
  theme_minimal() +
  theme(
...
########## CROSS VALIDATION GRAPH FOR MODELS A1, B1, C1 ##########

### Static Plot
combined_graph <- ggplot() +
  geom_line(data = bin_counts_A1, aes(x = bin, y = bin_percentage, color = "Model A1"), size = 1) +
  geom_line(data = bin_counts_B1, aes(x = bin, y = bin_percentage, color = "Model B1"), size = 1) +
  geom_line(data = bin_counts_C1, aes(x = bin, y = bin_percentage, color = "Model C1"), size = 1) +
  labs(x = "Prediction Error", y = "Percentage of Correct Predictions", title = "Baseline Dataset", color = "") +
  ...
  scale_color_manual(values = c("Model A1" = "steelblue", "Model B1" = "red", "Model C1" = "green")) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
combined_graph <- ggplot() +
  ...
  geom_line(data = bin_counts_B1, aes(x = bin, y = bin_percentage, color = "Model B1"), size = 1) +
  geom_line(data = bin_counts_C1, aes(x = bin, y = bin_percentage, color = "Model C1"), size = 1) +
  labs(x = "Prediction Error", y = "Percentage of Correct Predictions", title = "Baseline Dataset",
       color = "") +
  scale_x_continuous(breaks = seq(1, 50, by = 1), labels = paste0("±", abs(seq(1, 50, by = 1)))) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.0025), labels = scales::percent(seq(0, 1, by = 0.0025))) +
  scale_color_manual(values = c("Model A1" = "steelblue", "Model B1" = "red", "Model C1" = "green")) +
...
htmlwidgets::saveWidget(interactive_graph, file = "see Dataset 1 Baseline Dataset with all Models.html")
